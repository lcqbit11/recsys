...
W_shared = tf.Variable(tf.random_normal(shape=[number_feat, 100], mean=0, stddev=1), name='W_shared')
...
y_predict1 = tf.matmul(shared_out, W1) + b1  # 加权变换并进行softmax回归，得到预测概率
y_predict2 = tf.matmul(shared_out, W2) + b2  # 加权变换并进行softmax回归，得到预测概率
# y_predict1 = tf.nn.softmax(tf.matmul(shared_out,W1) + b1)     #加权变换并进行softmax回归，得到预测概率
# y_predict2 = tf.nn.softmax(tf.matmul(shared_out,W2) + b2)     #加权变换并进行softmax回归，得到预测概率
